...
//import ODD.ODDBranchMonitor.MyCreateStaticGraph2;
//import ODD.ODDBranchMonitor.MyLoadStaticGraph2;
//import ODD.ODDBranchMonitor.MyProcessEvents2;
import QL.Qlearner;
import soot.SootClass;

class MySocketServer2 extends Thread {   
	//private static final Logger logger=Logger.getLogger(ODDBranchMonitor.class); 	
    @Override
    public void run() {
        ...
        try {
	        ...
	        System.out.println("NIO2Server channel bound to port: " + hostAddress.getPort());
       	    //logger.info("ODDBranchMonitor NIO2Server channel bound to port: " + hostAddress.getPort());
            String query="";
            String impactSetStr="";
            while (true) {
            	String oldImpactSetStr="";
            	impactSetStr=ODDBranchMonitor.allQuestResults;
            	if (!oldImpactSetStr.equals(impactSetStr) && impactSetStr.length()>1) {
                	...
                	if (ODDBranchMonitor.configurations.equals("111111")) {
                ...
                {	
                	 impactSetStr=ODDBranchMonitor.getImpactSetStr(query,""+socketPort);
                	 System.out.println("impactSetStr = " + impactSetStr);
                	 //logger.info("ODDBranchMonitor impactSetStr = " + impactSetStr);
...
////	static boolean staticContextSensity = false;	
	...
	public synchronized static void initialize() throws Exception{
		...
		resetInternals();
		bInitialized = true;
		Thread queryServer = new MySocketServer2(); 
		queryServer.start();  
...
//		dynamicConfigurations=configurations.substring(2, 6);
		...
		System.out.println("**************ODDBranchMonitor 16_03::initialize()  0th");		
		...
		Runtime.getRuntime().addShutdownHook( new Thread()
        {
          public void run()
          {
    		...
    		try {
				...
				ODDBranchMonitor.terminate("Forced upon external termination");
...
//		dynamicConfigurations=configurations.substring(2, 6);
		...
		try {
			...
			System.out.println("ODDBranchMonitor initialization takes " + (System.currentTimeMillis() - startTime) + " ms");
			//logger.info("ODDBranchMonitor initialization took " + (System.currentTimeMillis() - startTime) + " ms");
    	} catch (Exception e) {
	...
	public static void getTimeoutsThresholds() throws Exception {
    	...
    	//logger.info("ODDBranchMonitor getTimeouts() staticLoadTimeOutTime=" + staticLoadTimeOutTime +" timeOutTimeDynamic=" + dynamicTimeOutTime+" staticCreateTimeOutTime=" + staticCreateTimeOutTime);
	}	
...
//	public static void MyProcessEvents() { 
        ...
        //logger.info("ODDBranchMonitor start processing events in the buffer of " + span_Queue.size() + " events...... ");
 	...
 	//[0] staticContextSensity  [1] staticFlowSensity  	[2] dynamicMethodInstanceLevel [3] dynamicStatementCoverage [4] staticGraph  [5]	dynamicMethodEvent	
    		...
    		System.out.println("ODDBranchMonitor MyProcessEvents after staticDynamicSettings[3]");
        	//logger.info("ODDBranchMonitor MyProcessEvents after staticDynamicSettings[3]");
    		...
    		System.out.println("ODDBranchMonitor MyProcessEvents after the computation");
        	//logger.info("ODDBranchMonitor MyProcessEvents after the computation");
        	//String allQuestResults="";
    		if (staticDynamicSettings[2])  {    		        //dynamicStaticGraph
    			//icAgent.dumpAllImpactSets();
    			//System.out.println("ODDBranchMonitor "+icAgent.getDumpAllImpactSetsSize());  
    			System.out.println("ODDBranchMonitor icAgent.getDumpAllImpactSetsSize()");
    			//logger.info("ODDBranchMonitor "+icAgent.getDumpAllImpactSetsSize());    
    			allQuestResults=icAgent.getDumpAllImpactSets();
    		}
    		else  {
    			//icAgent.dumpAllImpactSetsWithoutStatic();
    			//System.out.println("ODDBranchMonitor "+icAgent.getDumpAllImpactSetsSizeWithoutStatic());      
    			System.out.println("ODDBranchMonitor icAgent.getDumpAllImpactSetsSizeWithoutStatic()");
    			//logger.info("ODDBranchMonitor "+icAgent.getDumpAllImpactSetsSizeWithoutStatic());        			  
    		...
    		//lastEventTimeStamps.clear();
			  ...
			  System.out.println("MyProcessEvents is interrupted when calculating, will stop... Exception: "+e);
    			//logger.info("ODDBranchMonitor MyProcessEvents is interrupted when calculating, will stop...");  
			 return false;
		  }	    		
		return true;
	}			
	...
	public synchronized static void returnInto(String methodname, String calleeName) throws IOException, InterruptedException, ExecutionException{
		...
		try {
			...
			if (ifProcess) {
				...
				//logger.info("ODDBranchMonitor After g_counter="+g_counter+" g_counter_queue="+g_counter_queue+" span_Queue.size()"+span_Queue.size()+" B_Queueing.size()"+B_Queueing.size());
				//System.out.println("ODDBranchMonitor timeSpan="+timeSpan+" TIME_SPAN"+TIME_SPAN+" CN_LIMIT "+CN_LIMIT +" CN_LIMIT_QUEUE"+CN_LIMIT_QUEUE);
...
//		}
	...
	public synchronized static String getImpactSetStr(String query, String port) throws Exception {
 	...
 	//			timeOutTime= Long.MAX_VALUE/3;
 			//MyProcessEvents();
 	        MyProcessEvents task0 = new ODDBranchMonitor.MyProcessEvents();
 	    	//logger.info("ODDBranchMonitor ProcessEvents after staticUpdated");
 	        ...
 	        try {
 	            if (future1.get(dynamicTimeOutTime, TimeUnit.MILLISECONDS)) {  
 	    			...
 	    			//logger.info("ODDBranchMonitor ProcessEvents completes successfully");   		
 	            }
 	        } catch (InterruptedException e) {
 	            System.out.println("ProcessEvents was interrupted during the sleeping");
 				//logger.info("ODDBranchMonitor ProcessEvents was interrupted during the sleeping"); 
 	            executor.shutdownNow();
 	        } catch (ExecutionException e) {
 	            System.out.println("ProcessEvents has mistakes during getting the result");
 				//logger.info("ODDBranchMonitor ProcessEvents has mistakes during getting the result");
 	            executor.shutdownNow();
 	        } catch (TimeoutException e) {
 	            System.out.println("ProcessEvents is timeoouts");
 				//logger.info("ODDBranchMonitor ProcessEvents is timeoouts"); 
 	            future1.cancel(true);
 	             executor.shutdownNow();
 	             executor.shutdown();
 	        } finally {
 			...
 			//logger.info("ODDBranchMonitor updateTimeFromConfigurationToFileForced configurationFileName="+configurationFileName+" timeFileName="+timeFileName+" resultL"+resultL);
...
// 	        ODDUtil.writeStringToFileAppend(configurations+","+ resultL + "\n", timeCostsFileName);
 	     ...
 	     // With or without adaptation
 	       if (optionStr.toUpperCase().startsWith("Q")) 
 	        ...
 	        //logger.info("ODDBranchMonitor oldStaticConfigurations: " + oldStaticConfigurations + " newStaticConfigurations: " + staticConfigurations);
    ...
    public synchronized static long processEvents(long timeOutTime) throws  Exception{
		...
		//logger.info("ODDBranchMonitor Event computation took " + resultL + " ms");
...
//        }else 
        ...
        //logger.info("ODDBranchMonitor oldStaticConfigurations: " + oldStaticConfigurations);
...
//        }  
		...
		System.out.println("ODDBranchMonitor configurations="+configurations+" configurationFile"+configurationFile);
		staticConfigurations=configurations.substring(0, 2);
		//logger.info("ODDBranchMonitor configurations="+configurations);
		...
		for (int i=0; i<configurations.length(); i++)
		{
			...
			////logger.info("ODDBranchMonitor staticDynamicSettings["+i+"]="+staticDynamicSettings[i]);
		}	
	}
	...
	static class MyLoadStaticGraph2 implements Callable <Boolean> {
		@Override
        public Boolean call() throws Exception {
			try {
                Thread.sleep(1);     
				...
				if (initializeReult !=0)  {
	    			...
	    			//logger.info("ODDBranchMonitor Unable to load satic graph");
				}
				else
    ...
    public synchronized static void LoadStaticGraph(long timeOutTime) throws  Exception{
    	long startTime = System.currentTimeMillis();
        MyLoadStaticGraph2 task2 = new ODDBranchMonitor.MyLoadStaticGraph2();
        ExecutorService executor = Executors.newCachedThreadPool();
        ...
        try {
            if (f2.get(staticCreateTimeOutTime, TimeUnit.MILLISECONDS)) { // 
                ...
                //logger.info("ODDBranchMonitor Creating static graph successes");  
                isStaticCreateTimeOut=false;
            }
        } catch (InterruptedException e) {
	...
	static class MyCreateStaticGraph2 implements Callable <Boolean> {
		@Override
        public Boolean call() throws Exception {
			try {
				Thread.sleep(1); 
...
//					return false;
				...
				System.out.println("After ./ODDEventsStaticGraph.sh");				
				  ...
				  System.out.println("MyCreateStaticGraph is interrupted when calculating, will stop... Exception: "+e);
	    			//logger.info("ODDBranchMonitor MyInitialStaticGraph is interrupted when calculating, will stop...");
				  return false; // 
			  }
			return true;
		}
    ...
    public synchronized static void createStaticGraph(long timeOutTime) throws  Exception{
    	...
    	long startTime = System.currentTimeMillis();
        MyCreateStaticGraph2 task1 = new ODDBranchMonitor.MyCreateStaticGraph2();
